...
# ================== КОНФИГУРАЦИЯ ==================
TOKEN = "7991920232:AAEKMDzj0s4L8U81pNK4EVpeEazn0UoJYv0"  # Вставлен токен
...
def update_user_alco(user_id: int, username: str, added_volume: float, bottle_gained: bool):
    now = int(time.time())
    with get_db() as conn:
        # Получаем текущие данные или создаем нового
        cur = conn.execute(
        ...
        conn.commit()
        # Возвращаем обновлённый total_volume и bottles
        row = conn.execute(
...
async def casino_command(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    # Извлекаем число из сообщения (формат "казино 3")
    ...
    # Обновление БД (списываем все бутылки, добавляем выигранные литры)
...
def main():
    ...
    # Все остальные текстовые сообщения игнорируются (нет обработчика)
